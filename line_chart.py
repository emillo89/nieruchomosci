...
class LineChart(ReadData):
    ...
    def show_line_chart(self, w_city: str = 'All', w_kind_of_investment: str = 'All', w_market: str = 'All') -> \
        ...
        if w_city == 'All' and w_kind_of_investment == 'All' and w_market == 'All':
            data = self.df.groupby(['date_addition_add', 'year', 'month'], as_index=False).\
                agg({'price_per_1m2': pd.Series.mean}).sort_values(['year', 'month'])
        elif w_city == 'All' and w_kind_of_investment != 'All' and w_market == 'All':
            filtered_df = self.df.groupby(['date_addition_add', 'year', 'month', 'kind_of_investment']).\
                agg({'price_per_1m2': pd.Series.mean}).reset_index()
            data = filtered_df[(filtered_df['kind_of_investment'] == w_kind_of_investment)].\
                sort_values(['year', 'month'])
        elif w_city != 'All' and w_kind_of_investment == 'All' and w_market == 'All':
            filtered_df = self.df.groupby(['date_addition_add', 'year', 'month', 'city']).\
                agg({'price_per_1m2': pd.Series.mean}).reset_index()
            data = filtered_df[(filtered_df['city'] == w_city)].sort_values(['year', 'month'])
        elif w_city == 'All' and w_kind_of_investment == 'All' and w_market != 'All':
            filtered_df = self.df.groupby(['date_addition_add', 'year', 'month', 'market']).\
                agg({'price_per_1m2': pd.Series.mean}).reset_index()
            data = filtered_df[(filtered_df['market'] == w_market)].sort_values(['year', 'month'])
        elif w_city != 'All' and w_kind_of_investment != 'All' and w_market == 'All':
            ...
            filtered_df = self.df.groupby(['date_addition_add', 'year', 'month', 'kind_of_investment', 'market']).\
                agg({'price_per_1m2': pd.Series.mean}).reset_index()
            data = filtered_df[
            ...
            filtered_df = self.df.groupby(['date_addition_add', 'year', 'month', 'city', 'kind_of_investment',
                                           'market']).agg({'price_per_1m2': pd.Series.mean}).reset_index()
            data = filtered_df[
                (filtered_df['city'] == w_city) & (filtered_df['kind_of_investment'] == w_kind_of_investment) & (
