...
class Flats(Base):
    ...
    children2 = relationship('LinksDim', back_populates='parent')
...
class LinksDim(Base):
    __tablename__ = 'scd'
    id = Column(Integer, primary_key=True)
    link_id = Column(Integer, ForeignKey('property.link_id'))
    parent = relationship('Flats', back_populates='children2')
    kind_of_investment = Column(String(100), nullable=True)
    city = Column(String(100), nullable=False)
    area = Column(Float(), nullable=False)
    price = Column(Float(), nullable=False)
...
engine = create_engine('sqlite:///link4.db', echo=True)
# metadata = sqla.MetaData(bind=engine)
# datatable = sqla.Table('link', metadata, autoload=True)
Base.metadata.create_all(bind=engine)
Session = sessionmaker(bind=engine)
